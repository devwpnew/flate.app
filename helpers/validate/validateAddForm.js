const validateAddForm = (formData, sectionId, product, floorCheck) => {
  let emptyFields = {};
  let scrollToFields = {};
  let check = true;
  ...
  for (const key in tmpForm) {
    ...
    if (!tmpForm?.section_relation || (key === "section_relation" && !value)) {
      check = false;
      emptyFields.section_relation = `Поле "Тип объекта"`;
    ...
    if (
      ...
      check = false;
      emptyFields.property_product_address = `Поле "Адрес — выберите нужный адрес из выпадающего списка"`;
    ...
    if (sectionId === 3) {
      ...
      if (
        tmpForm?.property_product_galery?.length == 0 ||
    ...
    if (sectionId === 4) {
      ...
      if (
        ...
        check = false;
        emptyFields.house_communication = `Поле "Коммуникации"`;
        scrollToFields.about = 1;
      }
