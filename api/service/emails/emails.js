...
export default EMAILS = {
    ...
    newUser: async function sendNewUserEmail(idAdd, registeredFrom = 'Приложение') {
        if (!idAdd) {
        ...
        const res = await API.sendEmailNotification.send(mailFields)
        return res
    },
    ...
    feedback: async function sendFeedbackEmail(data, window_host) {
        ...
        if (data instanceof FormData) {
            fields.text_why = data.get('text')
            fields.text_idea = data.get('text2')
            fields.contact = data.get('text3')
        } else {
            fields.text_why = data.text
            fields.text_idea = data.text2
            fields.contact = data.text3
        }
